...
internal class CoupleNamesEditorExecutor @Inject constructor(
	...
	override fun executeAction(action: CoupleNamesEditorStore.Action, getState: () -> CoupleNamesEditorStore.State) {
		when (action) {
			else -> {}
		}
	}
	
	override fun executeIntent(intent: CoupleNamesEditorStore.Intent, getState: () -> CoupleNamesEditorStore.State) {
		when (intent) {
			else -> {}
		}
	}
