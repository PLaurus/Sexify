...
internal class _M_NAME_PASCAL_Executor @Inject constructor(
	...
	override fun executeAction(action: _M_NAME_PASCAL_Store.Action, getState: () -> _M_NAME_PASCAL_Store.State) {
		when (action) {
			else -> {}
		}
	}
	
	override fun executeIntent(intent: _M_NAME_PASCAL_Store.Intent, getState: () -> _M_NAME_PASCAL_Store.State) {
		when (intent) {
			else -> {}
		}
	}
